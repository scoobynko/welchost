...
def test_info_inline_is_single_line_with_accent_and_separator(fake_home):
    ...
    # Values only (no "user"/"host" labels), joined by "  ·  ".
    assert t.plain == f"{getpass.getuser()}  ·  {socket.gethostname()}"
...
def test_info_stacked_is_multiline_values_only(fake_home):
    ...
    assert t is not None
    # One bare value per line — labels dropped here too.
    assert t.plain == f"{getpass.getuser()}\n{socket.gethostname()}"
